...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/17 15:06:07 by ozouine           #+#    #+#             */
/*   Updated: 2023/12/17 16:27:55 by ozouine          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > FOPEN_MAX)
		return (NULL);
	str[fd] = buffer_reader(str[fd], fd);
	if (!str[fd])
		return (NULL);
	line = readline(str[fd]);
	str[fd] = line_remover(str[fd]);
	if (!str[fd])
